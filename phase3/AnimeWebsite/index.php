...
$resultAnime = mysqli_query($dbhandle,"SELECT title,genre,rating,description,season,a_status,start_date,studio,num_of_episodes,img from anime ORDER BY RAND() LIMIT 1");
//query to check for a random Manga recommendation
$resultManga = mysqli_query($dbhandle,"SELECT title,genre,description,rating,author,volume,published_date,img from manga ORDER BY RAND() LIMIT 1");
//query to check for a random Character recommendation
$resultCharacter = mysqli_query($dbhandle,"SELECT cname,description,rating,img from my_character ORDER BY RAND() LIMIT 1");
